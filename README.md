# Гра "Міста"

## Описання
Проект є реальзаціїю гри "Міста", в якій люди називають назви міст, які починаються на останню букву попереднього.

## Контекст
Проект виконаний по технічному завданню(ТЗ)

## Стек
Мова- Java
Бібліотеки:
java.awt
java.swing
- try-with-resources (робота з файлами)
- enum та власні класи результатів

## Як запустити
1. Склонувати репозиторій
   git clone<link>
2. Перейти в папку проекта
cd <name>
3. Переконайтесь, що встановлено Java 17 (JDK 17)
4. Запустіть проєкт:
   - через IDE (наприклад IntelliJ IDEA), або
   - запустіть головний клас (Main)
   
## Приклади роботи

### Cторінка привітання
![Main menu](screenshots/welcome.png)

### Ігровий процес
![Gameplay](screenshots/first-screen-of-game.png)
![Gameplay](screenshots/user-turn.png)
![Gameplay](screenshots/computer-answer.png)
![Gameplay](screenshots/if-gamer-write-city-that-have-been-used.png)
![Gameplay](screenshots/city-doesn't-exsist-or-not-in-file.png)

### Вікно перемоги / поразки
![Win](screenshots/win-screen.png)
![loose](screenshots/lose-screen.png)

## Що зробив я

- реалізував логіку створення вікон (UI) та взаємодії з користувачем
- розділив проєкт на пакети (стартове вікно, ігрова логіка)
- реалізував логіку гри: перевірка відповідей користувача, обробка перемоги та поразки
- додав генерацію назв міст для комп’ютера
- реалізував перевірку введених даних (існування міста, правильність назви)
- додав обробку дії "здаюсь"
- реалізував читання списку міст із файлу
- оптимізував роботу з файлами та структуру коду
- збільшив базу міст до 100+
## Покращення від себе
-Додав кнопки для початку Нової гри та для програшу користувача з прописаною логікою для них
- покращив структуру проєкту (розділення логіки на пакети)
- оптимізував роботу з файлами
- розширив базу даних міст для більшої варіативності гри
  
## Виправлення та покращення

Після отримання фідбеку було внесено такі зміни:

- додано перевірку правильної літери для ходу користувача після відповіді комп’ютера
- виправлено завершення гри (гра коректно завершується після перемоги або поразки)
- замінено String-результати ("WIN", "INVALID" тощо) на enum
- додано нормалізацію вводу (регістр, пробіли)
- покращено роботу з файлами (IOException, try-with-resources)
- видалено невикористані залежності

## Дані(міста)

Усі назви міст нормалізуються:
- переводяться у нижній регістр
- очищаються від пробілів

Це дозволяє коректно обробляти введення користувача незалежно від формату.

## 🧹 Якість коду

- Розділення логіки гри та UI
- Використання enum замість рядків
- Обробка помилок через IOException
- Використання try-with-resources для читання файлів
  
